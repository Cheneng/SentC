...
def test_BiLSTMAttn(model_num=1):
    ...
    GPU_NUM = 0
    MODEL_PATH = './checkpoint/BiLSTMAttn/base_seq2seq_epoch{}.ckpt'
    BATCH_SIZE = 200
    ...
    config = BiLSTMAttnConfig()
    model = BiLSTMAttn(config).cuda()
    ...
    for i, (src, trg, labels) in enumerate(testloader):
        ...
        for index in range(trg.shape[1]):
            ...
            out, hidden = model.attn_step_decoding(decoder_input, encoder_output, hidden)
        ...
        z = (predict_labels == ground_truth).long()
        correct_num += torch.sum(z).item()
