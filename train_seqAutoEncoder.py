...
# sequence auto encoder model to train
def train_SeqAutoEncoder(reload_dataset=False, epoch_num=50, pretrain_model_path=None,
                         optim_fu='adam', visdom_env='Sequence AutoEncoder'):
    ...
    SAVE_MODEL_PATH = './checkpoint/SeqAutoEncoder/'
    ...
    config = SeqAutoEncoderConfig()
    model = SeqAutoEncoder(config)
    ...
    if RELOAD_DATASET is True:
        data = dataset.SeqAutoEncoderDataset(vocab=vocab, data_path=DATA_DIR, reverse_src=True)
    ...
    for epoch in range(epoch_num):
        for src, trg in trainloader:
            ...
            flag4decoder = torch.zeros(trg.shape[0], trg.shape[1], 3).cuda()
            trg = torch.cat([trg, flag4decoder], dim=2)

            out, _ = model(src, trg)
            out = out.view(-1, config.out_feature_num)
            ...
            mask_matrix = (labels != config.pad_flag_index)
...
def test_output_file(save_dir='./output_file', save_filename='myTest.txt'):
    ...
    for index, (src, trg) in enumerate(testloader_word, 1):
        in_word = src.cuda()
        src = embed(src.cuda())
        trg = embed(trg.cuda())
        labels = labels.cuda()
