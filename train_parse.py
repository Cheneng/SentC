...
parser.add_argument('--embedding_parse', type=str, help='parse of embedding',
                    default='./model/save_parse_97.ckpt')
...
EMBEDDING_PATH_PARSE = args.embedding_parse

FLAG = 'transformer_parse'
EPOCH = 100
LR = args.lr
...
if os.path.exists(EMBEDDING_PATH_PARSE) is False:
    torch.save(embed_parse.state_dict(), EMBEDDING_PATH_PARSE)
else:
    embed_parse.load_state_dict(torch.load(EMBEDDING_PATH_PARSE))
...
data = dataset.CompresParseDataset(vocab=vocab, data_path=DATA_DIR)
...
for epoch in range(EPOCH):
    ...
    for index, (src, parse, trg, labels) in enumerate(trainloader, 1):
        ...
        parse = embed_parse(parse)

        # try only encoder to add the parse
        src += parse
