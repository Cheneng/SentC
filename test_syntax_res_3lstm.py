...
from config import LSTMSyntaxResConfig
from model import LSTM3LayersResSyntax
from helper import *
...
def test_GraphAttention():
    ...
    config = LSTMSyntaxResConfig()
    model = LSTM3LayersResSyntax(config)
    model.load(MODEL_PATH)
    model.cuda()

    # trainset = SyntaxDataset(vocab=vocab, data_path='./data/dataset_eval')
    ...
    for index, (src, trg, labels, tags) in enumerate(tqdm(trainloader)):
        ...
        remain_num += torch.sum(predict_labels).item()
