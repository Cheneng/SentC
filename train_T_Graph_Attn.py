...
from config import BiLSTMTGraphAttnConfig
from model import BiLSTMTGAttn
from graph_rank_dataset import GraphDataset
...
def train(reload_dataset=False, pretrain_model_path=None, optim_fu='adam'):
    ...
    SAVE_DATASET_OBJ = './data/dataset_graph.pkl'
    SAVE_MODEL_PATH = './checkpoint/BiLSTM_T_Graph_Attn/'
    VISDOM_ENV = 'Bi-LSTM-T-Graph-Atten-with'
    PRINT_STEP = 10
    SAVE_STEP = 1
    GPU_NUM = 0
    ...
    config = BiLSTMTGraphAttnConfig()
    model = BiLSTMTGAttn(config)
    ...
    for epoch in range(config.epoch):
        for index, (src, trg, degree, labels) in enumerate(tqdm(trainloader)):
